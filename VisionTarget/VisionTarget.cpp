...
cv::Mat lock_target(cv::Mat source, std::vector<std::vector<cv::Point>> contours, int width)
{
	...
	if(num_contours > 2){
		for(int count = 0; count < num_contours; count+2) {
			if(count+1<num_contours)
			{
...
int main(){
	...
	//network tables setup
  	while(true) {
		...
		if ( source.rows > 0){
			std::cout << "processed source" << std::endl;
			...
			if(!contours.empty() ){
				// cv::Mat rect_output = detect_rectangles(source,contours, cWidth);
