...

{
        ...
        cs::CvSink *cvSink_pointer = new cs::CvSink("opencv_USB Camera");
        cs::CvSink cvSink = *cvSink_pointer;
        cvSink.SetSource(camera);

        // set up processed camera streams on port 5801 (FloorLines) and 5802 (HSV stream)

        // cs::CvSource outputStreamStd = frc::CameraServer::GetInstance()->PutVideo("Floorlines", width, height);
        // cs::CvSource outputStreamHSV = frc::CameraServer::GetInstance()->PutVideo("HSV Stream", width, height);

        cs::MjpegServer *mjpegServer2_pointer = new cs::MjpegServer("Floor Lines", 5801);
        cs::MjpegServer mjpegServer2 = *mjpegServer2_pointer;

        cs::MjpegServer *mjpegServer3_pointer = new cs::MjpegServer("HSV Stream", 5802);
        cs::MjpegServer mjpegServer3 = *mjpegServer3_pointer;

        cs::CvSource *outputStreamStd_pointer = new cs::CvSource("Floor Lines", cs::VideoMode::PixelFormat::kMJPEG, width, height, fps);
        cs::CvSource outputStreamStd = *outputStreamStd_pointer;
        mjpegServer2.SetSource(outputStreamStd);

        cs::CvSource *outputStreamHSV_pointer = new cs::CvSource("HSV Stream", cs::VideoMode::PixelFormat::kMJPEG, width, height, fps);
        cs::CvSource outputStreamHSV = *outputStreamHSV_pointer;
        mjpegServer3.SetSource(outputStreamHSV);
